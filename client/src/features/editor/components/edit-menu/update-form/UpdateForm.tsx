...
import { Pencil, PencilIcon, PlusIcon, Trash2Icon, X } from 'lucide-react';
import { useUpdateForm } from '../../../hooks/edit-menu/useUpdateForm';
import { IComponent, IDesign } from '@/types/design.types';
import { IFileInfo, INestedChildLevel1, INestedChildLevel2, INestedParentLevel1, INormalComponent } from '@/types/project.types';
...
import { getComponentLockStatus, getOptionLockStatus, isOptionLocked } from '@/features/editor/utils/ComponentTracker';
...
const UpdateForm: React.FC = () => {
    ...
    const componentPath = menuOf.slice(0, -1).join(".");
    const lockStatus = (designSnapshot && updatedValue && isDesignMode)
        ? menuOf.length === 1
            ? getComponentLockStatus(menuOf[menuOf.length - 1], updatedValue as (IComponent | INormalComponent), designSnapshot)
            : getOptionLockStatus(componentPath, menuOf[menuOf.length - 1], designSnapshot)
            ...
            if (option !== "none") {
                ...
                return (
                    <UpdateChild
                        ...
                        option={option}
                        value={value}
                        componentPath={menuOf.join('.')}
                    />
                );
            }
            return null;
        });
    }, [selectedComponentValue, updatedValue, setUpdatedValue, designSnapshot, isDesignMode, menuOf]);
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex gap-2 ">
                                        ...
                                        <Input
                                            ...
                                            className={`flex-1 ${!canRename && "bg-amber-100/70 border-amber-300 hover:border-amber-600"}`}
                                        />
                                    </div>
                                </div>
