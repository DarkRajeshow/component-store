...
function ActionBar({ generatePDF }) {
    ...
    const { loading, components, uniqueFileName, setUniqueFileName, design, fetchProject, selectedCategory, toggleAttributeValue, pushToUndoStack, handleUndo, handleRedo } = useStore();
    ...
    const handleAttributeFileNameChange = useCallback(() => {
        ...
        let updatedcomponents = JSON.parse(JSON.stringify(components));
        ...
        switch (attributeType) {
            ...
            case 'nestedParentLevel1':
                if (levelOneNest) {
                    ...
                    updatedcomponents[levelOneNest] = {
                        selectedOption: updatedcomponents[levelOneNest]?.selectedOption,
    ...
    const memoizedcomponents = useMemo(() => {
        return components && Object.entries(components).sort(([a], [b]) => a.localeCompare(b)).map(([attribute, value]) => (
    ...
    }, [components, openDropdown, menuVisible, handleToggle, toggleDropdown, handleToggleContextMenu, pushToUndoStack]);
        ...
        <Dialog className='rounded-lg col-span-3 overflow-hidden h-[10vh]'>
            <div className="pt-4 flex items-center justify-between px-6 pb-2 select-none"
                ...
                <div className="flex gap-1 rounded-md h-[88%] justify-center" ref={contextMenuRef}>
                    {!loading && components && memoizedcomponents}
                </div>
