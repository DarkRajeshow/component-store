...
function DeleteForm() {
    ...
    const { menuOf, components, setComponents, generateStructure, setUndoStack, setRedoStack } = useStore();
    const { id } = useParams();
    ...
    const deleteValue = () => {
        if (menuOf.length === 3) {
            if (tempcomponents[menuOf[0]].options[menuOf[1]].selectedOption === menuOf[menuOf.length - 1]) {
                tempcomponents[menuOf[0]].options[menuOf[1]].selectedOption = " "
            }
            delete tempcomponents[menuOf[0]].options[menuOf[1]].options[menuOf[menuOf.length - 1]];
        } else if (menuOf.length === 2) {
            if (tempcomponents[menuOf[0]].selectedOption === menuOf[menuOf.length - 1]) {
                tempcomponents[menuOf[0]].selectedOption = "none"
            }
